...
public class QueryUtils {
    ...
    public static List<Newsnetwork> fetchNewsData(String requestUrl) {
        ...
        List<Newsnetwork> newsList = extractFeatureFromJson(jsonResponse);
    ...
    private static List<Newsnetwork> extractFeatureFromJson(String newsJSON) {
        if (TextUtils.isEmpty(newsJSON)) {
            return null;
        }

        List<Newsnetwork> newsList = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < resultsArray.length(); i++) {
                ...
                Newsnetwork news = new Newsnetwork(Title, category, date, url, author);
                newsList.add(news);
            }
